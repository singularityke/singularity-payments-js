...
describe("MpesaReactClient", () => {
  let client: MpesaReactClient;
  let mockFetch: ReturnType<typeof vi.fn>;

  beforeEach(() => {
    mockFetch = vi.fn();
    client = new MpesaReactClient({
  ...
  describe("Constructor", () => {
    it("should initialize with default config", () => {
      const defaultClient = new MpesaReactClient();
      expect(defaultClient).toBeInstanceOf(MpesaReactClient);
    });

    it("should initialize with custom baseUrl", () => {
      const customClient = new MpesaReactClient({
        baseUrl: "https://custom.api.com",
      });
      expect(customClient).toBeInstanceOf(MpesaReactClient);
    });

    it("should strip trailing slash from baseUrl", async () => {
      const clientWithSlash = new MpesaReactClient({
        baseUrl: "https://api.example.com/",
        fetcher: mockFetch as typeof fetch,
      });
