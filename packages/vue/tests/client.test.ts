...
import { MpesaVueClient } from "../src/client";
import {
  mockSTKPushResponse,
...
describe("MpesaVueClient", () => {
  let client: MpesaVueClient;
  let mockFetch: ReturnType<typeof vi.fn>;

  beforeEach(() => {
    mockFetch = vi.fn();
    client = new MpesaVueClient({
  ...
  describe("Constructor", () => {
    it("should initialize with default config", () => {
      const defaultClient = new MpesaVueClient();
      expect(defaultClient).toBeInstanceOf(MpesaVueClient);
    });

    it("should initialize with custom baseUrl", () => {
      const customClient = new MpesaVueClient({
        baseUrl: "https://custom.api.com",
      });
      expect(customClient).toBeInstanceOf(MpesaVueClient);
    });

    it("should strip trailing slash from baseUrl", async () => {
      const clientWithSlash = new MpesaVueClient({
        baseUrl: "https://api.example.com/",
        fetcher: mockFetch as typeof fetch,
      });
